 ...
 */
export const checkAny = (items: string | string[], resourcePool: string[]) => {
	return Array.isArray(items) ? items.some((item) => check(item, resourcePool)) : check(items, resourcePool);
};
